...
export const validateBodyForAdd = async (
    ...
    if (req.method !== 'POST') {
        res.status(405).json({
            code: 'vitruveo.studio.api.allowList.validateBodyForAdd.failed',
            message: '',
            transaction: nanoid(),
        } as APIResponse);
...
export const validateBodyForUpdate = async (
    ...
    if (req.method !== 'PUT') {
        res.status(405).json({
            code: 'vitruveo.studio.api.allowList.validateBodyForUpdate.failed',
            message: '',
            transaction: nanoid(),
        } as APIResponse);
...
export const validateBodyForCreate = async (
    ...
    if (req.method !== 'POST') {
        res.status(405).json({
            code: 'vitruveo.studio.api.allowList.validateBodyForCreate.failed',
            message: '',
            transaction: nanoid(),
        } as APIResponse);
...
export const validateBodyForCreateMultiple = async (
    ...
    if (req.method !== 'POST') {
        res.status(405).json({
            code: 'vitruveo.studio.api.allowList.validateBodyForCreateMultiple.failed',
            message: '',
            transaction: nanoid(),
        } as APIResponse);
