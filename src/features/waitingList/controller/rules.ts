...
import {
    ...
    schemaValidationForCreateMultiple,
...
export const validateBodyForAdd = async (
    ...
    if (req.method !== 'POST') {
        res.status(405).json({
            code: 'vitruveo.studio.api.waitingList.validateBodyForAdd.failed',
            message: '',
            transaction: nanoid(),
        } as APIResponse);
...
export const validateBodyForUpdate = async (
    ...
    if (req.method !== 'PUT') {
        res.status(405).json({
            code: 'vitruveo.studio.api.waitingList.validateBodyForUpdate.failed',
            message: '',
            transaction: nanoid(),
        } as APIResponse);
...
export const validateBodyForCreate = async (
    ...
    if (req.method !== 'POST') {
        res.status(405).json({
            code: 'vitruveo.studio.api.waitingList.validateBodyForCreate.failed',
            message: '',
            transaction: nanoid(),
        } as APIResponse);
...
export const validateBodyForCreateMultiple = async (
    req: Request,
    res: Response,
    next: NextFunction
) => {
    if (req.method !== 'POST') {
        res.status(405).json({
            code: 'vitruveo.studio.api.waitingList.validateBodyForCreateMultiple.failed',
            message: '',
            transaction: nanoid(),
        } as APIResponse);
    ...
    try {
        req.body = req.body.map((item: { email: string }) => ({
            ...item,
        ...
        req.body = schemaValidationForCreateMultiple.parse(req.body);
